...
namespace KikoleSiteUnitTests.Models.Requests
{
    public class PlayerRequestTests
    {
        ...
        {
            var request = Valid();
            request.AllowedNames = new List<string> { "Zidane", "  " };
        ...
        {
            var request = Valid();
            request.Clubs = Career(positions);
        ...
        {
            var request = Valid();
            request.ProposalDate = Today.AddDays(-1);
        ...
        {
            var request = Valid();
            request.ProposalDate = Today.AddDays(daysAhead);
        ...
        {
            var request = Valid();
            request.ProposalDate = Today.AddHours(3);
        ...
        {
            var request = Valid();
            request.HideCreator = hide;

            request.ToDto(42).HideCreator.Should().Be(expected);
        }
